...
<CrossCourseRef
  ...
  lessonSlug="09-structured-streaming/04-watermarks-late-data"
  label="Watermarks в Spark Structured Streaming"
  preview="Как watermark работает в Spark: семантика, late data handling, append/update mode, отличия от Flink."
/>

<CrossCourseRef
  courseId="spark-course"
  lessonSlug="09-structured-streaming/05-windowing"
  label="Windowing в Structured Streaming"
  preview="Tumbling/sliding/session windows, window aggregations, dropDuplicates с watermark, ловушки stateful agg."
/>
...
<CrossCourseRef
  ...
  preview="transformWithState — гибкая stateful обработка: TTL, multiple state types, timer-based callbacks. Аналог Flink ProcessFunction."
/>
...
<KnowledgeCheck>
Stream processing: Flink (stateful), Kafka (event bus), event time + watermarks (correctness), windowing (tumbling/sliding/session), checkpointing (exactly-once). Flink 2.0 (март 2025) — disaggregated state через ForSt: state в S3/HDFS, локальный hybrid cache, unlimited state size, fast recovery. Patterns: enrichment, deduplication, DLQ. В следующем модуле: Data Warehouse Design.
</KnowledgeCheck>
